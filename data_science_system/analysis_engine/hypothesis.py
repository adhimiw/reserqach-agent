...
class HypothesisGenerator:
    ...
    def format_hypotheses_for_report(self) -> str:
        ...
        for htype, hypotheses in by_type.items():
            ...
            for h in hypotheses:
                report += f"### {h['hypothesis']}\n\n"
                report += f"**Why:** {h['reasoning']}\n\n"
                report += f"**Test Method:** {h['test_method']}\n\n"
                
                # Add relevant details based on type
                if htype == "correlation":
                    report += f"**Correlation Value:** {h['correlation_value']}\n"
                    report += f"**Strength:** {h['strength']}\n"
                    report += f"**Direction:** {h['direction']}\n\n"
                elif htype == "distribution":
                    report += f"**Metric:** {h['metric']}\n"
                    report += f"**Value:** {h['value']}\n\n"
                elif htype == "categorical":
                    report += f"**Unique Values:** {h['unique_values']}\n\n"
                elif htype == "outlier":
                    report += f"**Outlier Count:** {h['outlier_count']}\n"
                    report += f"**Percentage:** {h['outlier_percentage']}%\n"
                    report += f"**Severity:** {h['severity']}\n\n"
                elif htype == "trend":
                    report += f"**Trend Type:** {h['trend_type']}\n"
                    report += f"**Strength:** {h['strength']}\n"
                    report += f"**R-Squared:** {h['r_squared']}\n\n"
