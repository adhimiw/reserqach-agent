...
var handleToolsCall = async (state, params, ctx) => {
  ...
  const sampling = ctx.clientCapabilities?.sampling && requestFn ? {
    createMessage: async (samplingParams) => {
      const result = await requestFn(Method.SamplingCreateMessage, samplingParams);
      return result;
    }
  } : undefined;
  const elicit = ctx.clientCapabilities?.elicitation && requestFn ? async (message2, schema) => {
    const result = await requestFn(Method.ElicitationCreate, {
      message: message2,
...
var createServer = (config) => {
  ...
  const handle = async (input, options) => {
    ...
    if (isRequest(parsed.value) && parsed.value.method === Method.Initialize && parsed.value.params) {
      const initParams = parsed.value.params;
...
var http = (options = {}) => {
  return (server, _notify) => {
    ...
    const jsonRpcRoute = post(basePath, async ({ ctx }) => {
      try {
        ...
        try {
          ...
          if (isJsonRpcResponse(parsed2) && session) {
            const pending = session.pendingRequests.get(parsed2.id);
            if (pending) {
              session.pendingRequests.delete(parsed2.id);
              clearTimeout(pending.timer);
              if ("error" in parsed2 && parsed2.error) {
                const errObj = parsed2.error;
                pending.reject(new Error(errObj.message ?? "Request failed"));
              } else {
                pending.resolve(parsed2.result);
              }
        ...
        if (acceptsSSE) {
          ...
          const parsedReq = parseMessage(body);
          if (parsedReq.ok && isRequest(parsedReq.value) && parsedReq.value.method === "initialize") {
          ...
          const notify = (method, params) => {
            ...
            eventQueue.push({ data: stringify(notification2), event: "message" });
            eventResolve?.();
          };
          const request2 = async (method, params) => {
            if (!activeSession)
            ...
            eventQueue.push({ data: stringify(req), event: "message" });
            eventResolve?.();
            return promise;
          };
          const handlerPromise = server.handle(body, { notify, request: request2 }).then((result) => {
            handlerResult = result;
            handlerComplete = true;
            eventResolve?.();
          });
          const generateSSE = async function* () {
          ...
          if (activeSessionId && !sessionId) {
            headers2["Mcp-Session-Id"] = activeSessionId;
          }
          return sse(generateSSE, { headers: headers2 });
        }
    ...
    const healthRoute = get(`${basePath}/health`, () => json2({
      status: "ok",
      server: server.name,
      version: server.version
    }));
    const routes = [jsonRpcRoute, healthRoute];
    const middleware = options.cors ? cors({
      origin: options.cors,
  ...
  return (server, _notify) => {
    ...
    const start = async () => {
      ...
      const request2 = async (method, params) => {
        ...
        writer.write(encoder.encode(`${stringify(req)}
`));
        writer.flush();
        return promise;
      };
...
`)) {
            ...
            try {
              ...
              if ("id" in parsed && parsed.id !== null && typeof parsed.id === "string" && parsed.id.startsWith("server-")) {
            ...
            try {
              const response2 = await server.handle(line, { request: request2 });
              if (response2 && writer) {
